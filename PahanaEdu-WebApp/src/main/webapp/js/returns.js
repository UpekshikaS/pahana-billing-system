(() => {
    ...
    const renderTable = (items) => {
        ...
        items.forEach(item => {
            ...
            row.innerHTML = `
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                            <input type="number" class="w-20 p-1 border rounded-md text-center return-qty-input" value="1" min="1" max="${item.quantityPurchased}" data-id="${item.invoiceItemId}">
                        </td>
        ...
        invoiceNumberDisplay.textContent = items[0].invoiceNumber;
    ...
    const openReturnModal = (invoiceItemId, productName) => {
        ...
        if (isNaN(quantityToReturn) || quantityToReturn <= 0 || quantityToReturn > maxQuantity) {
            showNotification(`Invalid quantity. Please enter a number between 1 and ${maxQuantity}.`, 'error');
            return;
        }
    ...
    function showNotification(message, type = 'success', duration = 4000) {
        ...
        notification.className = 'fixed top-6 right-6 z-50 max-w-sm w-full rounded-md shadow-lg p-4 text-white';
