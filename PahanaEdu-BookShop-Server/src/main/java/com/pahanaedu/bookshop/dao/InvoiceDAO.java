...
public class InvoiceDAO {
    ...
    public int createInvoice(Invoice invoice) throws SQLException {
        ...
        try {
            ...
            try (PreparedStatement pstmtItem = conn.prepareStatement(insertInvoiceItemSQL); PreparedStatement pstmtStock = conn.prepareStatement(updateStockSQL)) {

                for (InvoiceItem item : invoice.getInvoiceItems()) {
                    pstmtStock.setInt(1, item.getQuantity());
                    pstmtStock.setInt(2, item.getProductId());
                    pstmtStock.setInt(3, item.getQuantity()); // Check if stock is sufficient
